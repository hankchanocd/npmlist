const { sortByDate, parseDate } = require("../../build/utils/dateUtil");
...
describe("sortByDate()", () => {
	...
	test("Sorts the list against the date correctly", () => {
		let mockList = [
			mockObjGenerator("1997-01-26"),
			mockObjGenerator("2015-01-26"),
			mockObjGenerator("2013-01-26")
		];
...
describe("parseDate()", () => {
	...
	test("previous years return the correct format: month-day hour:min, year", () => {
		...
		let hour = date.getHours();
		let min = date.getMinutes();
		expect(parseDate(date)).toBe(
			`${month}-${day} ${hour}:${min} ${date.getFullYear()}`
		);
	});
